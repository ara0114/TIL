...
//for of
for(let fruit of fruits){//객체는 in 배열은 of
    console.log(fruit);
}
//forEach
...
//fruits.splice(1,1);
//splice(삭제 시작 할 인덱스, 삭제갯수(지정하지않으면 모두), 인덱스앞으로추가할데이터)
fruits.splice(1,0,'배','수박');//1인덱스에서 삭제없이 배, 수박 추가하겠다.
console.log(fruits);
...
//join: 배열을 하나의 문자열로 만들기, toString과 같은 역하를 하지만 특별한 구분자 사용가능
...
```javascript
//splice: 배열자체를 분리, slice 원래배열은 냅두고 새로운 배열 생성
